...
//        查询用户拥有的徽章
        List<UserBackpack> userBadges = userBackpackCache.getByItemIds(uid, badges.stream().map(ItemConfig::getId).collect(Collectors.toList()));
//        查询用户佩戴的徽章
        User byUid = userCache.getByUid(uid);
//        构建返回对象
...
//        查询用户是否拥有该徽章
        UserBackpack userBackpack = userBackpackCache.getByItemId(uid, req.getBadgeId());
//        用户没有此徽章，抛出异常
        AssertUtil.isNotEmpty(userBackpack,"用户没有此徽章");
//        判断物品是否是徽章
        ItemConfig itemConfig = itemCache.getById(req.getBadgeId());
//        不是徽章，抛出异常
...
//            清空缓存
            userCache.delByUid(uid);
            userBackpackCache.delItemCount(uid,ItemEnum.MODIFY_NAME_CARD.getId());
...
//        拉黑用户
        userCache.delByRegion(update.getId());
